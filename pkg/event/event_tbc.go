...
// is no longer LOCKED (fn is not called).
...
func (e *EventHandler) downstreamAnnounceIWF(ctx context.Context, cfgName string) {
	...
	if err != nil {
		glog.Error("Failed to fetch upstream data, downstream data can not be updated: ", err)
		return
	}
	...
	e.applyIfLockedBC(cfgName, "after downstream announce", func() {
		e.LeadingClockData.downstreamParentDataSet = &results.ParentDataSet
		e.LeadingClockData.downstreamTimeProperties = &results.TimePropertiesDS
	})
}
	...
	if data, ok := e.data[cfgName]; ok {
		for _, d := range data {
			if d.ProcessName == PTP4l {
				...
				ptp4lOffset := int64(d.window.Mean())
				if worstOffset == FaultyPhaseOffset {
					worstOffset = ptp4lOffset
				} else if math.Abs(float64(ptp4lOffset)) > math.Abs(float64(worstOffset)) {
					worstOffset = ptp4lOffset
				}
				continue
			}
